...
class TareaModel extends CI_Model
{
    ...
    public function registrar($descripcion, $categoria)
    {
        $this->db->insert('tareas', ['descripcion' => $descripcion, 'categoria' => $categoria, 'completada' => 0]);
    }

    public function completar($id)
    {
        $this->db->where('id', $id);
        $this->db->update('tareas', ['completada' => 1]);
    }
    ...
    public function actualizar($id, $descripcion, $categoria)
    {
        $this->db->where('id', $id);
        $this->db->update('tareas', ['descripcion' => $descripcion, 'categoria' => $categoria]);
    }
}
    */
...
class TareaModel extends CI_Model
{
    ...
    public function mis_tareas($user_id)
    {
        $this->db->where('user_id', $user_id); // Filtrar por user_id
        $this->db->order_by('categoria', 'ASC');
        $this->db->order_by('descripcion', 'ASC');
        return $this->db->get('tareas')->result();
    }

    // Registrar tarea asociándola al usuario
    public function registrar($user_id, $descripcion, $categoria)
    {
        $data = [
            'user_id' => $user_id,
    ...
    // Antes de completar, verificar que la tarea pertenece al usuario (opcional pero seguro)
    public function es_mi_tarea($tarea_id, $user_id)
    {
        $this->db->where('id', $tarea_id);
        $this->db->where('user_id', $user_id);
        return $this->db->count_all_results('tareas') > 0;
    }

    public function completar($id, $user_id) // Ahora también necesitas user_id
    {
        if ($this->es_mi_tarea($id, $user_id)) {
            $this->db->where('id', $id);
            // $this->db->where('user_id', $user_id); // Doble seguridad, ya validado arriba
            $this->db->update('tareas', ['completada' => 1]);
            return true;
        }
        return false;
    ...
    public function eliminar($id, $user_id) // Ahora también necesitas user_id
    {
        if ($this->es_mi_tarea($id, $user_id)) {
            $this->db->delete('tareas', ['id' => $id, 'user_id' => $user_id]); // Asegurar que solo borra sus tareas
            return true;
        }
        return false;
    }

    public function actualizar($id, $user_id, $descripcion, $categoria) // Ahora también necesitas user_id
    {
        if ($this->es_mi_tarea($id, $user_id)) {
            $this->db->where('id', $id);
            // $this->db->where('user_id', $user_id);
